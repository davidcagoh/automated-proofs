...
---
> **Version 3 changes (relative to v2).** Three Lean formalization issues identified from the Aristotle v2 submission (job `0739c66d`) and corrected:
>
> 1. **`gradient_projection` bug (fixed).** The RHS had an extra `V.mulVec` application: `ρ_r* • V W̄ Σˣˣ v_r*` instead of the correct `ρ_r* • W̄ Σˣˣ v_r*`. Corrected to match the paper statement $V^\top(\rho_r^* I - V)\bar{W}\Sigma^{xx}\mathbf{v}_r^*$.
>
> 2. **`quasiStatic_approx` (Lemma 5.2) — Lean hypotheses updated.** The v2 Lean file still used the weak hypothesis `hV_flow : deriv(‖V - V_qs‖) ≤ 0` (pointwise derivative non-positive) instead of the explicit ODEs from the v2 paper. Replaced with three explicit hypotheses matching v2 paper sections H1–H3: (H1) encoder moves slowly (`‖Ẇ̄(t)‖_F ≤ Kε²`), (H2) decoder satisfies `Ṽ = -∇_V ℒ` with `HasDerivAt`, (H3) off-diagonal amplitudes ≤ `K ε^{1/L}`. The proof strategy (Phases A and B) remains unchanged.
>
> 3. **`diagonal_ODE` (Prop 6.1) — statement strengthened after disproof.** Aristotle confirmed the v2 Lean statement is mathematically false: `σ_r` was a free function with only a scalar bound, allowing counterexample `σ_r(t) = t²`. In v3, `σ_r(t)` is explicitly linked to the gradient flow: `σ_r t = diagAmplitude dat eb (Wbar t) r` with `Wbar` satisfying the preconditioned gradient flow (via a `HasDerivAt` hypothesis on `σ_r` itself), `V` quasi-static (Lemma 5.2 conclusion as hypothesis), and off-diagonal amplitudes bounded via `offDiagAmplitude dat eb (Wbar t) r s`. These match hypotheses (H-diag) and (H-offdiag) stated in this section.
>
> 4. **`offDiag_ODE` (Lemma 7.1) — statement strengthened analogously.** Same issue as `diagonal_ODE`: `c_{rs}` and `σ_r, σ_s` were free functions. In v3, `c_{rs}(t) = offDiagAmplitude dat eb (Wbar t) r s` and similarly for `σ_r, σ_s`, with a `HasDerivAt` hypothesis encoding the preconditioned off-diagonal gradient flow ODE.
>
> 5. **`JEPA_rho_ordering` main theorem — vacuous hypothesis replaced.** The placeholder `h_Wbar : ∀ t, True` is replaced with proper gradient-flow hypotheses (`hWbar_slow`, `hWbar_init`, `hV_flow_ode`, `hV_init`) so that Parts A–B can be derived from the other lemmas rather than remaining sorry by default.

# JEPA Learns Influential Features First
## A Proof Without Simultaneous Diagonalizability

**David Goh — March 2026 · Version 3**
...
[v3: Lean hypotheses match paper: (H1) replaced by `hWbar_slow` (`‖Ẇ̄(t)‖_F ≤ Kε²`) and `hWbar_init`; (H2) expressed as `HasDerivAt V (-gradV dat (Wbar t) (V t)) t`; (H3) off-diagonal bound via `offDiagAmplitude`. The proof sketch below is unchanged.]
...
[v3: Lean statement now requires $\sigma_r(t) = u_r^{*\top}\bar{W}(t)\mathbf{v}_r^*$ where $\bar{W}(t)$ satisfies the preconditioned gradient flow, expressed via a `HasDerivAt` hypothesis on $\sigma_r$ directly. The quasi-static decoder bound (Lemma 5.2 conclusion) and off-diagonal bound (Theorem 7.3) are explicit hypotheses (H-diag) and (H-offdiag) rather than derived inside the proof.]
...
[v3: Main theorem now takes explicit hypotheses `hWbar_slow`, `hWbar_init`, `hV_flow_ode`, `hV_init` replacing the vacuous `h_Wbar : ∀ t, True`. Part (A) follows directly from `quasiStatic_approx` applied to these hypotheses plus the off-diagonal bound from Part (B).]
